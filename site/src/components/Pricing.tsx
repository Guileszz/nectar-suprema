  ...
  return (
    <section className="py-20 px-4 bg-blue-900 text-white">
      <div className="max-w-4xl mx-auto text-center">
        <h2 className="text-4xl md:text-5xl font-black mb-6 uppercase italic">{data.title}</h2>
        <div className="bg-white text-gray-900 p-8 md:p-12 rounded-3xl shadow-2xl relative overflow-hidden">
          ...
          <p className="text-xl text-gray-500 line-through mb-2">{data.old_price}</p>
          <p className="text-2xl font-bold text-gray-700">{data.price_label}</p>
          <div className="text-6xl md:text-8xl font-black text-blue-600 my-4">
            {data.new_price}
          </div>
          <p className="text-xl font-medium mb-10 text-gray-600 italic">{data.installments_label}</p>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 text-sm font-bold text-gray-500">
            {data.guarantees?.map((g: string, i: number) => (
              <span key={i} className="flex items-center gap-2">
                {i === 0 ? <ShieldCheck className="text-green-600" /> : <CheckCircle className="text-green-600" />}
                {g}
              </span>
            ))}
          </div>
        ...
        {data.bonuses && (
          <div className="mt-16 text-left">
            <h3 className="text-2xl font-bold mb-8 text-center">{data.bonuses_title}</h3>
            <div className="grid md:grid-cols-2 gap-4">
              {data.bonuses.map((bonus: string, i: number) => (
